...
#define PM_HAS_UNIFORM_TYPE_IDENTIFIERS 1
#else
#define PM_HAS_UNIFORM_TYPE_IDENTIFIERS 0
#endif

static NSString *PMPreferredMimeTypeForUTI(NSString *uti) {
    if (!uti.length) {
        return nil;
    }
#if PM_HAS_UNIFORM_TYPE_IDENTIFIERS
    if (@available(iOS 14.0, macOS 11.0, *)) {
        ...
        if (type.preferredFilenameExtension.length > 0) {
            return [@"application/" stringByAppendingString:type.preferredFilenameExtension];
        }
    }
#endif
    ...
    return (__bridge_transfer NSString *)mime;
#else
    return nil;
#endif
#else
...
- (NSString *)mimeType {
    ...
    if (resource) {
        ...
        return PMPreferredMimeTypeForUTI(uti);
    }
    return nil;
}
