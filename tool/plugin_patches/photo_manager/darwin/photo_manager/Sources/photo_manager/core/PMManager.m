...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        BOOL success = [[UIApplication sharedApplication] openURL:settingsURL];
#pragma clang diagnostic pop
        [result reply:@(success)];
    }
#else
...
- (void) cancelRequestWithCancelToken:(NSString *)cancelToken {
    ...
    if (requestId) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [strongSelf.cachingManager cancelImageRequest:requestId.intValue];
        });
        [requestIdMap removeObjectForKey:cancelToken];
    }
}
...
- (void) cancelAllRequest {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSMutableDictionary<NSString *, NSNumber *> *map = strongSelf->requestIdMap;
        for (NSString *key in map) {
            NSNumber *requestId = map[key];
            [strongSelf.cachingManager cancelImageRequest:requestId.intValue];
        }
    });
