...
- (UIWindow *)pm_activeWindow {
    ...
    if ([appDelegate respondsToSelector:@selector(window)]) {
        UIWindow *legacyWindow = appDelegate.window;
        if (legacyWindow) {
            return legacyWindow;
        }
    }
...
- (UIViewController *)pm_visibleControllerFrom:(UIViewController *)controller {
    if (!controller) {
        return nil;
    }
    ...
    if ([current isKindOfClass:[UINavigationController class]]) {
        return [self pm_visibleControllerFrom:((UINavigationController *)current).visibleViewController];
    }
    if ([current isKindOfClass:[UITabBarController class]]) {
        return [self pm_visibleControllerFrom:((UITabBarController *)current).selectedViewController];
    }
    return current;
}

- (UIViewController *)getCurrentViewController {
    UIWindow *window = [self pm_activeWindow];
    UIViewController *root = window.rootViewController;
    return [self pm_visibleControllerFrom:root];
}
