...
static id FLTNullToNil(id value) {
...
- (void)initializeAppAppName:(nonnull NSString *)appName
  ...
  options.APIKey = FLTNullToNil(initializeAppRequest.apiKey);
  options.projectID = FLTNullToNil(initializeAppRequest.projectId);

  // kFirebaseOptionsDatabaseUrl
  NSString *databaseURL = FLTNullToNil(initializeAppRequest.databaseURL);
  if (databaseURL) {
    options.databaseURL = databaseURL;
  }

  // kFirebaseOptionsStorageBucket
  NSString *storageBucket = FLTNullToNil(initializeAppRequest.storageBucket);
  if (storageBucket) {
    options.storageBucket = storageBucket;
  }

  // kFirebaseOptionsIosBundleId
  NSString *bundleId = FLTNullToNil(initializeAppRequest.iosBundleId);
  if (bundleId) {
    options.bundleID = bundleId;
  }

  // kFirebaseOptionsIosClientId
  NSString *clientId = FLTNullToNil(initializeAppRequest.iosClientId);
  if (clientId) {
    options.clientID = clientId;
  }

  // kFirebaseOptionsAppGroupId
  NSString *appGroupId = FLTNullToNil(initializeAppRequest.appGroupId);
  if (appGroupId) {
    options.appGroupID = appGroupId;
  }

  NSString *authDomain = FLTNullToNil(initializeAppRequest.authDomain);
  if (authDomain != nil) {
    customAuthDomains[appNameIos] = authDomain;
  }
