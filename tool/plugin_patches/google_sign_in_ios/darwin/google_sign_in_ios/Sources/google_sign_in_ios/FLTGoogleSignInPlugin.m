...
@property(nonatomic, assign) BOOL hasLoadedGoogleServiceProperties;
...
- (instancetype)initWithViewProvider:(NSObject<FSIViewProvider> *)viewProvider {
  return [self initWithSignIn:[[FSIGIDSignInWrapper alloc] init]
...
- (instancetype)initWithSignIn:(NSObject<FSIGIDSignIn> *)signIn
  ...
  if (self) {
    ...
    _hasLoadedGoogleServiceProperties = googleServiceProperties != nil;
...
- (GIDConfiguration *)configurationWithClientIdentifier:(nullable NSString *)runtimeClientIdentifier
  ...
  NSDictionary<NSString *, id> *googleServiceProperties = self.googleServiceProperties;
  if (!googleServiceProperties &&
      (runtimeClientIdentifier == nil || runtimeServerClientIdentifier == nil)) {
    googleServiceProperties = [self loadGoogleServicePropertiesIfNeeded];
  }

  NSString *clientID = runtimeClientIdentifier ?: googleServiceProperties[kClientIdKey];
  if (!clientID) {
    // Creating a GIDConfiguration requires a client identifier.
    return nil;
  }
  NSString *serverClientID =
      runtimeServerClientIdentifier ?: googleServiceProperties[kServerClientIdKey];
...
- (NSDictionary<NSString *, id> *)loadGoogleServicePropertiesIfNeeded {
  if (!self.hasLoadedGoogleServiceProperties) {
    self.googleServiceProperties = FSILoadGoogleServiceInfo();
    self.hasLoadedGoogleServiceProperties = YES;
  }
  return self.googleServiceProperties;
}
