...
class SecTrustProxyAPIDelegate: PigeonApiDelegateSecTrust {
    ...
    -> [SecCertificateWrapper]?
  {
    if #available(iOS 15.0, macOS 12.0, *) {
      guard let array = secTrustCopyCertificateChain(trust.value) as? [SecCertificate] else {
        return nil
      }
      return array.map { SecCertificateWrapper(value: $0) }
    }
#if os(macOS)
    ...
    if count > 0 {
      ...
      for index in 0..<count {
        if let certificate = secTrustGetCertificateAtIndex(trust.value, index) {
...
#if os(macOS)
  // Overridable for testing (macOS fallback only).
