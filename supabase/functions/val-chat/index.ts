...
class ValliError extends Error {
  ...
  constructor(code: ValliError["code"], message?: string, payload?: unknown) {
    super(message ?? code);
    this.code = code;
    this.payload = payload;
    this.name = "ValliError";
  }
}
...
function safeParseSlotFillingResponse(raw: string): any | null {
  try {
    ...
    const parsed = safeParseJson(raw, fallback, 'slot_filling_response');
    ...
    // Проверяем наличие обязательных полей и подставляем дефолты
    if (!parsed.updated_slots || typeof parsed.updated_slots !== 'object') {
      parsed.updated_slots = {};
    }
    if (typeof parsed.suggested_step_index !== 'number') {
      parsed.suggested_step_index = 1;
    }
    if (typeof parsed.bot_response_text !== 'string') {
      parsed.bot_response_text = '';
    }
...
const AI_TASK_REQUEST_TYPE: Record<AiTaskType, string> = {
  onboarding: 'valli_onboarding',
  dialog_generator: 'valli_generator',
  dialog_validator: 'valli_validator',
  slot_filling: 'valli_slot_filling',
  scoring: 'valli_scoring',
  hint: 'valli_hint',
} as const;
...
// ============================
// ValliEngine – Orchestrator
// ============================

class ValliEngine {
  private ctx: ValliContext;

  constructor(ctx: ValliContext) {
    this.ctx = ctx;
  }
  ...
  async handleScore(
    ...
    const { json } = await executeAiTask(this.ctx, {
      taskType: 'scoring',
      userHint: 'scoring of full valli conversation',
      model: CONFIG.MODELS.DEFAULT,
      messages: [
    ...
    const scoringResult = json || jsonFallback;
...
Проанализируй ответ пользователя и обнови состояние слотов.`;
  ...
  const rawResult = json || jsonFallback;

  const updatedSlots = (rawResult.updated_slots &&
    typeof rawResult.updated_slots === 'object'
    ? rawResult.updated_slots
    : {}) as Partial<Record<keyof SlotsState['slots'], Partial<SlotData>>>;

  const suggestedStepIndex =
    typeof rawResult.suggested_step_index === 'number'
      ? rawResult.suggested_step_index
      : currentStep;

  const botResponseText =
    typeof rawResult.bot_response_text === 'string'
      ? rawResult.bot_response_text
      : '';
...
function navigateNextStep(
  ...
  // Soft Validation: если после нескольких попыток AI не двигает шаг вперёд,
...
async function validateUserResponse(
  ...
  // ШАГ 1: Generator — генерируем ответ Валли через общий AI-сервис
  const model = CONFIG.MODELS.DEFAULT;
  const ctx: ValliContext = {
    aiClient: openai,
    dbAdmin: supabaseAdmin,
  ...
  const generatorResult = await executeAiTask(ctx, {
    ...
    userHint: 'legacy step-by-step valli dialog generator',
    model,
    messages: [
...
Оцени, достаточен ли ответ для перехода к следующему вопросу.`;
  ...
  try {
    ...
    const { json: validationResult } = await executeAiTask(ctx, {
      taskType: 'dialog_validator',
      userHint: 'legacy valli answer sufficiency validator',
      model,
      messages: [
...
async function updateValidationState(
  ...
  if (update.slotsState) {
    payload.slots_state = update.slotsState;
  }
  if (typeof update.currentStep === 'number') {
    payload.current_step = Math.min(update.currentStep, MAX_STEPS);
  }
  if (typeof update.retryCount === 'number') {
    payload.retry_count = update.retryCount;
  }
  ...
  try {
    ...
    if (error) {
      console.error('ERR update_validation_state', {
...
async function spendGP(
  ...
  amount: number,
  /**
   * Идемпотентный ключ операции списания.
   * ДОЛЖЕН быть стабильным между повторными попытками (например, validation_${validationId}).
   * Генерацию ключа оставляем на вызывающую сторону, чтобы избежать двойного списания при ретраях.
   */
  ...
  try {
    ...
    if (spendError) {
      ...
      if (spendError.message?.includes('insufficient') || spendError.code === '23514') {
        throw new ValliError('insufficient_gp', 'Недостаточно GP для валидации идеи', {
          required: amount,
        });
      }

      throw new ValliError('gp_error', 'Ошибка списания GP');
    }
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'dialog' && action === 'start_validation') {
      ...
      if (!pricing.isFree) {
        // Списываем GP (репозиторий сам выбросит ValliError при недостаточном балансе)
    ...
    // Build Valli context for downstream orchestration and logging
    const ctx: ValliContext = {
      aiClient: openai,
      dbAdmin: supabaseAdmin,
      dbUser: supabaseUser,
    ...
    const engine = new ValliEngine(ctx);
...
function handleGlobalError(error: unknown): Response {
  ...
  if (error instanceof ValliError) {
    const baseBody: any = {
      error: error.code,
      message: error.message,
    };
