...
function safeParseSlotFillingResponse(raw: string): any | null {
  try {
    ...
    if (result && result._parseError === true) {
      console.warn('WARN slot_filling_parse_error', {
        rawSnippet: result._rawSnippet || raw.slice(0, 200),
        context: result._context || 'slot_filling_response',
...
class ValliEngine {
  ...
  async handleScore(
    ...
    if (json && json._parseError === true) {
      ...
      alertParseError(
        'scoring',
        'scoring of full valli conversation',
        json._rawSnippet,
        validationId,
        userId,
      );
 ...
 * 
 * Использует SQL функцию update_validation_atomic(), которая:
 * - Обновляет slots_state, current_step, retry_count в одной транзакции
 * - Проверяет принадлежность записи пользователю
 * - Выбрасывает исключение при ошибке (автоматический rollback)
 * 
 * Преимущества перед обычным UPDATE:
 * - Атомарность: либо все поля обновляются, либо ничего
 * - Безопасность: проверка прав доступа на уровне БД
 * - Простота: один вызов вместо цепочки обновлений
 */
async function updateValidationState(
  ...
  try {
    ...
    const { error } = await supabaseAdmin.rpc('update_validation_atomic', {
      ...
      p_user_id: userId,
      // Передаём NULL для полей, которые не нужно обновлять
      // SQL функция использует COALESCE для сохранения текущих значений
    ...
    if (error) {
      ...
      throw error; // Пробрасываем ошибку, чтобы вызывающая сторона знала о проблеме
    }
    ...
    // Пробрасываем ошибку дальше, чтобы вызывающая сторона могла обработать
    throw e;
  }
}
