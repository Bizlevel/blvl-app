## Оглавление
- 2026-01-22 — Исправления: баги 3/5/8/9 + Back в L4/L7
- 2026-01-22 — Стандартизация клавиатуры и ввода
- 2026-01-21 — Багфикс серии 1–10 (мобайл) + Supabase
...
### Аутентификация (Bug #1)
- `AuthService.signInWithGoogle`: стабилизирован Google Sign‑In на iOS/Android, убран двойной prompt, добавлена безопасная валидация токенов.

### Рефералы и промокоды (Bug #2)
- Профиль: блок промокодов упрощён до одного экрана, статусы ошибок перенесены в inline‑сообщения, добавлена кнопка «Поделиться» с диплинком.
- Supabase: подготовлена миграция `20260121_fix_referral_code_search_path.sql` для корректного `search_path` в `get_referral_code`.

### Главная и уведомления (Bug #3)
- Добавлен корневой `ScaffoldMessenger` через `rootScaffoldMessengerKey`, баннеры больше не исчезают при пересборках.
- Карточки главной (`Goal/Quote/Continue`) показывают предыдущие данные во время фоновой загрузки.

### Уровень 0 — профиль (Bug #4)
- Убрана «карандашная» правка в L0, аватар и поля доступны только при первичном заполнении.

### Башня и покупки этажей (Bug #5)
- Кнопка покупки показывается только при блокировке по GP (не при незавершённых уровнях).
- Разблокировка учитывает реальный `floor_number`, убран хардкод этажа 1.

### Чат Лео/Макса (Bug #6)
- Добавлены breadcrumbs на открытие/закрытие, модалки стали `barrierDismissible: false`.
- После закрытия чата возвращаем пользователя на исходный маршрут (особенно `/tower`).

### Повтор уровня (Bug #8)
- Разрешён повтор завершённых уровней независимо от статуса предыдущих.
- Добавлена кнопка «Повторить уровень» с локальным reset прогресса.

### Артефакты (Bug #9)
- Подсчёт артефактов унифицирован с экраном артефактов (только `is_completed=true`).

### Чекпоинты и цель/Макс (Bug #10)
- L4/L7: кнопка «Завершить» теперь записывает флаги в `user_goal` и инвалидацию провайдеров.
- Башня: статусы чекпоинтов синхронизированы с `financial_focus` и `action_plan_note`.
- Проверены связи: экран «Цель», контекст Макса и чекпоинты теперь используют один источник истины.

### Supabase (проверки)
- Проверены RLS политики и схема `user_goal`/`practice_log`.
- Просмотрены логи `edge-function`: вызовы `leo-chat` возвращают 200 без ошибок.

### Git/документация
- Создан коммит и пуш в ветку `fix/keyboard-input-standardization-2026-01-17`.
- В коммит включены все актуальные изменения, а также чистка/перенос docs.

## 2026-01-22 — Стандартизация клавиатуры и ввода

### Единые пресеты ввода
- Добавлен `TextFieldPreset` (`auth/form/chat`) в `CustomTextBox` с безопасными дефолтами для автокоррекции, подсказок и капитализации.
- `BizLevelTextField` получил `preset` (по умолчанию `form`) для统一 поведения в формах.

### Логин/регистрация
- Приведены к единому `auth`‑пресету: одинаковые типы клавиатуры, отключены подсказки и автокоррекция.

### Цель и журнал применений
- Для текстовых полей цели/заметок включены form‑настройки (`sentences` + подсказки).
- Для числовых полей метрик отключены подсказки/автокоррекция.

### Чаты Лео/Рэй
- Ввод приведён к общему паттерну: `TextCapitalization.sentences` + подсказки, стабильный send‑ввод.

### Безопасное поведение клавиатуры
- Bottom sheets с вводом унифицированы через `showBizLevelInputBottomSheet`, inset от клавиатуры учитывается всегда.
- Для ключевых экранов ввода добавлены `resizeToAvoidBottomInset` и `keyboardDismissBehavior: onDrag`.

## 2026-01-22 — Исправления: баги 3/5/8/9 + Back в L4/L7

### Баг 3 — уведомления на главной
- Ошибки открытия уровня на главной переведены на `NotificationCenter` (устойчивые баннеры).
- GP‑уведомления в журнале применений и профиле показываются через `NotificationCenter`.

### Баг 5 — покупка этажей без prerequisites
- В `levels_provider` `lockReason` теперь учитывает `previousCompleted`, “Требуются GP” показывается только если prerequisites выполнены.
- Логика `previousCompleted` учитывает `userCurrentLevelNumber`, чтобы не блокировать повторное прохождение при отсутствии старых записей.

### Баг 8 — повтор уровня 1
- Добавлен флаг `isRepeatable` и разрешён повтор, если уровень уже пройден или пользователь пошёл дальше.
- Кнопка «Повторить уровень» и доступ к уровню основаны на `isRepeatable`, а не только `isCompleted`.

### Баг 9 — артефакты
- Счётчик артефактов на профиле ограничен уровнями 1..10 и только `is_completed=true`, синхронизирован с экраном артефактов.

### Back в L4/L7
- В `LeoDialogScreen` embedded‑режим больше не блокирует pop; назад теперь работает в чекпоинтах L4/L7.
