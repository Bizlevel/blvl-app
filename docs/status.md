...
# Этап 28: Фича «Цель»
- Задача 28.1: создана миграция `20250812_28_1_create_goal_feature_tables.sql` (таблицы `core_goals`, `weekly_progress`, `reminder_checks`, `motivational_quotes`).
- Включён RLS и политики owner-only; добавлены триггеры `set_user_id`, `updated_at` и guard для редактирования только последней версии цели.
- Добавлен сид из 50 цитат в `motivational_quotes` (идемпотентный upsert).
- Миграция применена к Supabase (проект `acevqbdpzgbtqznbpgzr`) через supabase-mcp, ошибок не обнаружено.
