...
эЗадача 40.8 fix: Добавлен фича‑флаг `kDisableGpSpendInChat` (rollback). При включении чат работает без списаний («Временно бесплатно»), ядро GP/контракты не менялись. Добавлены безопасные breadcrumbs `gp_spend_skipped`.