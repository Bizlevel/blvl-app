...
Задача 30.3: В `ProfileScreen` добавлена серая шестерёнка в `SliverAppBar` (PopupMenu). Пункты: Настройки (пока snackbar‑заглушка), Платежи (переход на `/premium`), Выход (вызов `AuthService.signOut`). Минимальные изменения, логику существующих секций не менял, линты без ошибок.
