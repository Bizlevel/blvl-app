...
Задача 30.4: В `LeoChatScreen` чипы заменены на две карточки (Leo AI / Max AI) по центру; активная карточка подсвечивается, при нажатии происходит переключение и перезагрузка списка чатов. FAB показывает мини‑аватар и надпись «Новый чат с …». Линты чистые, изменения минимальные.