...
эЗадача 40.3 fix: Подключены идемпотентные бонусы на клиенте: signup_bonus при первом входе; profile_completed после полной карточки профиля. Баланс обновляется фоном, ошибки бонусов не пробрасываются. Контракты GP/чат не изменялись.
эЗадача 40.4 fix: Магазин GP — удалён ручной ввод purchase_id: сохраняем last_purchase_id в Hive при init, кнопка «Проверить» выполняет auto-verify. Тестовый mock продолжает auto-verify сразу. UX/контракты без изменений.
эЗадача 40.5 fix: Наблюдаемость GP — добавлены breadcrumbs без PII: gp_insufficient (списание/открытие этажа, чат). В чате пишет where=leo_*; в списании — type/amount. Контракты/логика не менялись.
эЗадача 40.6 fix: Тесты — добавлен лёгкий тест кеша `GpService` (save/read). Сложные unit/widget по GP оставлены без изменений (покрываются существующими интеграциями), сборка/линты чистые.