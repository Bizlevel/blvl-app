...
Проверка Этапа 35 (мини‑кейсы): добавлены узлы `mini_case` в `towerNodesProvider` с гейтингом следующего уровня до `completed/skipped`; `BizTowerScreen` уже поддерживал переход на `/case/:id`. `MiniCaseScreen` открывает диалог с `caseMode=true`; в `LeoDialogScreen` системный промпт кейса теперь реально передаётся (как `system`). В Supabase подтверждена схема (`mini_cases`, `user_case_progress`, RLS, сиды 1/2/3 активны). Мини‑кейсы работают, лимиты чатов не тратятся на клиенте.