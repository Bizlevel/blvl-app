...
- Что сделать:
  1) В шапке оставить счётчик сообщений для активного бота.
  2) Вместо `_buildBotSwitcher()` отрисовать две карточки одинакового размера:
     - Лео: аватар, «Leo AI», подпись серым «Твой бизнес‑ментор».
     - Макс: аватар `avatar_max.png`, «Max AI», подпись серым «Твой помощник в достижении цели».
