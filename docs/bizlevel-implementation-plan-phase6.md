...
### Задача 25.8: QA и логи
- **Инструменты:** sentry-mcp, supabase-mcp
- **Что делать:**
  1. Прогнать приложение (Web/iOS/Android), проверить последовательность блоков уровня 0 и переход на Уровень 1.
  2. Проверить логи Sentry на отсутствие новых критических ошибок, игнорируя известные не-критичные (см. `scripts/sentry_check.sh`).
  3. Проверить через `supabase-mcp` advisors, что нет регрессий RLS/индексов.
- **Проверка:** логи чистые от критических ошибок; advisors без новых предупреждений по безопасности/производительности.
